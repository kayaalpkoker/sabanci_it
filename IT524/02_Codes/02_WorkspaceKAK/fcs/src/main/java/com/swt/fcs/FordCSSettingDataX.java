...
public class FordCSSettingDataX {
	...
	public boolean validateCurrent(float current) { //change type
		if (current >= 0 &&  current <= 30) return true;
		return false;
	}
	public boolean validateOTemperature(float f) {//-30 +50
		if (f >= -30 &&  f <= +50) return true;
		return false;
	}
	...
	public boolean validateCordLength(float cordLength) {
	...
	public boolean validateWeight(float weight) {
		if(weight==10.0) return true;
		return false;
	}
	public boolean validateRegulatory(String regulatory) {
		if (regulatory.equals(P_regulatory)) return true;
		return false;
	}
